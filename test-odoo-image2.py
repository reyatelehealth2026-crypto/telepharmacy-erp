...
for url in image_urls:
    try:
        ...
        with urllib.request.urlopen(req, timeout=8) as r:
            ...
            size = len(r.read())
            print(f"  ✅ {url}")
            print(f"     Content-Type: {ct}, Size: {size} bytes")
            break
...
# Also try JSONRPC to get image as base64
...
body = {
    ...
    "params": {
        ...
        "method": "read",
        "args": [[product_id], ["image_1920", "image_512", "image_128"]],
        "kwargs": {}
    }
}
...
try:
    with urllib.request.urlopen(req, timeout=10) as r:
        ...
        if result:
            rec = result[0]
            for k in ["image_1920", "image_512", "image_128"]:
                v = rec.get(k)
                if v and v is not False:
                    print(f"  {k}: base64 len={len(str(v))} chars")
                else:
                    print(f"  {k}: False/None")
        else:
